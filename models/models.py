...
class FeaturePredictNet(nn.Module):
    ...
    def serialize(model, optimizer, epoch, tr_loss=None, cv_loss=None):
        package = {
            ...
            'embedding_dim': model.embedding_dim, 'encoder_num_convs': model.encoder_num_convs,
            ...
            'postnet_num_convs': model.postnet_num_convs, 'postnet_filter_size': model.postnet_filter_size,
            'postnet_kernel_size': model.postnet_kernel_size,
            # state
...
class LocationSensitiveAttention(nn.Module):
    ...
    def forward(self, query, values, cumulative_attention_weights, mask=None):
        ...
        attention_context = torch.bmm(attention_weights.unsqueeze(1),
                                      values)  # [N, 1, Ti] bmm [N, Ti, He] -> [N, 1, He]
